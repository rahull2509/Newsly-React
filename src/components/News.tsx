...
const News: React.FC<NewsProps> = ({
  ...
  const [lastKeyIndex, setLastKeyIndex] = useState<number>(0);

  const capitalizeFirstLetter = (str: string): string =>
    str.charAt(0).toUpperCase() + str.slice(1);

  const truncateText = (text: string | undefined, limit: number, byWords = false): string => {
    if (!text) return "";
    if (byWords) {
  ...
  const fetchNews = useCallback(
    async (pageNumber: number, pageToken = ""): Promise<void> => {
      if (loading) return;
      setProgress(20);
      setLoading(true);
      ...
      for (let i = lastKeyIndex; i < apiKeys.length; i++) {
        ...
        const url = `https://newsdata.io/api/1/news?apikey=${apiKeys[i]}&country=in&language=en${categoryParam}${
      ...
      setArticles((prevArticles) =>
        pageNumber === 1
          ? parsedData.results || []
          : [...prevArticles, ...(parsedData.results || [])]
      );
  ...
  return (
    <div className="container my-3">
      <h2 className="text-center" style={{ margin: "35px 0px", marginTop: "80px" }}>
        Newsly - {category === "general" ? "Top" : capitalizeFirstLetter(category)} Headlines
      </h2>
        ...
        <div className="row">
          {articles.map((element, index) => (
            <div className="col-md-4" key={element.link || index}>
              <NewsItem
                title={element.title || ""}
                description={truncateText(element.description, 170)}
